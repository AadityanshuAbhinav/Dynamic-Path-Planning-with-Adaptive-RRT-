...
class RRTStar:
    ...
    def extract_path(self):
        ...
        # Combine with previous valid path
        full_path = self._prev_valid_path.copy()
        if full_path:
            full_path = full_path[:-1] + path  # Remove duplicate point
        else:
            full_path = path
    ...
    def draw(self, surface):
        ...
        if full_path:
            full_path = full_path[:-1] + self.path  # Remove duplicate point
        else:
            full_path = self.path
...
rrt = RRTStar(start, goal, obstacles, moving_obstacles)
if not rrt.find_path():
...
while running:
    ...
    # Handle path obstruction and replanning
    if not planning:
        ...
        if obstructed_index is not None:
            ...
            if rrt.path and 0 <= obstructed_index < len(rrt.path):
                last_safe_point = rrt.path[max(0, obstructed_index - 1)]
                last_safe_node = next((n for n in rrt.nodes if (n.x, n.y) == last_safe_point), None)
                
                if last_safe_node:
                    # Preserve valid path history
                    new_rrt = RRTStar((last_safe_node.x, last_safe_node.y), goal, obstacles, moving_obstacles)
                    new_rrt._prev_valid_path = rrt.path[:max(0, obstructed_index)]
                    new_rrt.nodes = [n for n in rrt.nodes if n.cost <= last_safe_node.cost]
                    new_rrt.nodes.append(last_safe_node)
                    rrt = new_rrt
                    planning = True
            else:
                # print("Invalid path state, triggering full replan")
                rrt = RRTStar(start, goal, obstacles, moving_obstacles)
                planning = True
    else:
